...
app.title = 'Impact des Acteurs, Réalisateurs et Studios sur le Succès des Films'
    ...
    try:
        ...
        try:
            ...
            directors = [
                ...
                if isinstance(member, dict) and member.get("job") == "Director" and "name" in member and "id" in member
            ]
            ...
            director_pattern = r'"job":\s*"Director"[^}]*"name":\s*"([^"]+)"[^}]*"id":\s*(\d+)'
        ...
        print(f"Erreur lors de l'extraction des réalisateurs (méthode alternative): {str(e)}")
        return []
...
def extract_studios(companies_json):
    ...
    try:
        ...
        print(f"Erreur lors de l'extraction des studios (méthode alternative): {str(e)}")
        return []
...
actors_df['entity_id'] = actors_df['cast_list'].apply(lambda x: x.get('id') if isinstance(x, dict) else None)
actors_df['entity_name'] = actors_df['cast_list'].apply(lambda x: x.get('name') if isinstance(x, dict) else None)
...
directors_df['entity_id'] = directors_df['directors_list'].apply(lambda x: x.get('id') if isinstance(x, dict) else None)
directors_df['entity_name'] = directors_df['directors_list'].apply(lambda x: x.get('name') if isinstance(x, dict) else None)
...
studios_df['entity_id'] = studios_df['studios_list'].apply(lambda x: x.get('id') if isinstance(x, dict) else None)
studios_df['entity_name'] = studios_df['studios_list'].apply(lambda x: x.get('name') if isinstance(x, dict) else None)
...
def aggregate_data(entity_df):
    ...
    agg_df = entity_df.groupby(['entity_id', 'entity_name']).agg({
...
print(f"Nombre de réalisateurs après filtrage (min {min_films} films): {len(directors_agg)}")
print(f"Nombre de studios après filtrage (min {min_films} films): {len(studios_agg)}")

# Vérification des valeurs moyennes de revenus
print(f"Revenu moyen max pour les acteurs: ${actors_agg['avg_revenue'].max()/1000000:.2f}M")
print(f"Revenu moyen max pour les réalisateurs: ${directors_agg['avg_revenue'].max()/1000000:.2f}M")
print(f"Revenu moyen max pour les studios: ${studios_agg['avg_revenue'].max()/1000000:.2f}M")
...
app.layout = html.Div(style={'font-family': 'Arial, sans-serif', 'margin': '0', 'padding': '0'}, children=[
    ...
    html.Header(style={
        ...
        html.H1('Impact des Acteurs, Réalisateurs et Studios sur le Succès des Films', style={'margin': '0'}),
        html.P("Analyse comparative des performances basée sur différentes métriques", 
        ...
        html.Div(style={
            ...
            html.Div(style={'margin-bottom': '20px'}, children=[
                ...
                dcc.RadioItems(
                    id='success-metric',
                    options=[
                        {'label': 'Revenu moyen par film', 'value': 'avg_revenue'},
                        {'label': 'Note moyenne', 'value': 'avg_rating'},
                        {'label': 'Indice de popularité moyen', 'value': 'avg_popularity'}
                    ],
                    value='avg_revenue',
                    labelStyle={'display': 'block', 'margin': '10px 0'}
                )
            ...
            # Slider pour le nombre d'entités à afficher
            html.Div(style={'margin-bottom': '20px'}, children=[
                html.Label('Nombre d\'entités à afficher:'),
                dcc.Slider(
                    id='entities-count-slider',
                    min=5,
                    max=200,
                    step=5,
            ...
            html.Div(id='data-info', style={'margin-top': '20px', 'font-size': '14px', 'display': 'none'})
        ]),
...
# Callback pour mettre à jour le graphique à barres
@callback(
    ...
    [Input('entity-type', 'value'),
     ...
     Input('entities-count-slider', 'value')]
)
def update_bar_chart(entity_type, metric, count):
    ...
    if metric == 'avg_revenue':
        ...
        top_entities['formatted_value'] = top_entities[metric].apply(lambda x: f"${x/1000000:.2f}M")
        hover_template = "%{y:$.2f}"
        metric_display = "revenu moyen"
    elif metric == 'avg_rating':
        y_title = "Note moyenne (/10)"
        top_entities['formatted_value'] = top_entities[metric].apply(lambda x: f"{x:.1f}/10")
        hover_template = "%{y:.1f}/10"
        metric_display = "note moyenne"
    else:  # avg_popularity
        y_title = "Indice de popularité moyen"
        top_entities['formatted_value'] = top_entities[metric].apply(lambda x: f"{x:.1f}")
        hover_template = "%{y:.1f}"
        metric_display = "popularité moyenne"
    ...
    fig.add_trace(go.Bar(
        x=top_entities['entity_name'],
        y=top_entities[metric],
        text=top_entities['formatted_value'],
        hoverinfo='text',
        hovertext=[
            f"<b>{row['entity_name']}</b><br>" +
            f"{metric_display.capitalize()}: {row['formatted_value']}<br>" +
            f"Nombre de films: {row['film_count']}"
    ...
    fig.update_layout(
        ...
        margin={
            'l': 50,
            'r': 20,
            'b': 100 if show_tick_labels else 50,
            't': 100
        },
        plot_bgcolor='white',
        hoverlabel={
    ...
    chart_info = html.Div([
        html.P(f"Ce graphique montre les {count} {entity_type_display.lower()} avec le {metric_display} le plus élevé"),
        html.P(f"Seules les entités ayant participé à au moins {min_films} films sont incluses")
    ])
    
    # Informations sur les données
    data_info = html.Div([
        html.P(f"Total {entity_type_display.lower()} avec ≥{min_films} films: {len(df_to_use)}"),
    ...
    return fig, chart_info, data_info

# Démarrage du serveur
if __name__ == '__main__':
    app.run(debug=True)

print("Exécutez cette application avec 'python app.py' et accédez à http://127.0.0.1:8050/ dans votre navigateur")